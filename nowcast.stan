...
transformed parameters {
  ...
  vector[t] infectiousness;          // infections over time
  vector[t] branch_infections;       // infections generated by the branching process
...
model {
  ...
  for (s in 2:t) {
   // centre noise on previous noise each time (today is like yesterday)
   noise[s] ~ normal(noise[s - 1], 0.5) T[0,];
  ...
  if (estimate_r) {
    ...
    target += normal_lpdf(infections | branch_infections, 0.01);
  }
}
