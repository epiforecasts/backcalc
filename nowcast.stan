functions {
  ...
  vector convolve(vector cases, vector pdf) {
   ...
   for (s in 1:t) {
     if (convolved_cases[s] == 0) {
       convolved_cases[s] = 0.000001;
     }
   }
...
parameters{
  ...
  vector<lower = 0>[7] day_of_week_eff;         // day of week reporting effect
...
model {
  ...
  for (s in 2:t) {
   // centre noise on previous noise each time and give tight prior (today is like yesterday)
   noise[s] ~ normal(noise[s - 1], 0.1) T[0,];
